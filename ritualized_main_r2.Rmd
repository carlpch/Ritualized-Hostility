...
grid.arrange(vconf_acf_plot, vconf_pacf_plot, ncol=2)
```
...
mconf.acf.plot <-mconf.acf$acf[-1] %>% 
  ...
  geom_segment(aes(xend=lags, yend=0)) +geom_point() + 
  geom_hline(yintercept = interval, linetype=2, size = 0.5, color="red", alpha=0.5) + theme_setting
mconf.pacf.plot <- vcoop.r.pacf$acf[-1] %>% 
  as_tibble() %>% mutate(lags = 1:n()) %>%
  ggplot(aes(x=lags, y = value)) + 
  geom_segment(aes(xend=lags, yend=0)) +geom_point() + theme_setting + 
   scale_x_continuous(breaks=seq(0,41,4))+ 
...
grid.arrange(mconf.acf.plot, mconf.pacf.plot, ncol=2)
...
dokdo_drill <- ritualized_day %>%
  mutate(
  `2012.02 Drill` = ifelse(as_date(time) %in% seq(as_date("2012/02/01"), by="days", length.out=pd),1,0),
  `2012.09 Drill` = ifelse(as_date(time) %in% seq(as_date("2012/09/07"), by="days", length.out=pd),1,0),
  `2013.06 Drill` = ifelse(as_date(time) %in% seq(as_date("2013/06/20"), by="days", length.out=pd),1,0),
  `2013.10 Drill` = ifelse(as_date(time) %in% seq(as_date("2013/10/25"), by="days", length.out=pd),1,0),
  `2014.06 Drill` = ifelse(as_date(time) %in% seq(as_date("2014/06/20"), by="days", length.out=pd),1,0),
  `2014.11 Drill` = ifelse(as_date(time) %in% seq(as_date("2014/11/24"), by="days", length.out=pd),1,0),
  `2015.05 Drill` = ifelse(as_date(time) %in% seq(as_date("2015/05/15"), by="days", length.out=pd),1,0)
  #`2015.11 Drill` = ifelse(as_date(time) %in% seq(as_date("2015/11/09"), by="days", length.out=pd),1,0),
  #`2016.06 Drill` = ifelse(as_date(time) %in% seq(as_date("2016/06/08"), by="days", length.out=pd),1,0),
  #`2016.12 Drill` = ifelse(as_date(time) %in% seq(as_date("2016/11/21"), by="days", length.out=pd),1,0)
  ) %>% 
  dplyr::select(-c(1:7))
...
```{r}
Verbal_conf<- auto.arima(ritualized_day$`Verbal Conflict`, test="adf", xreg = cbind(takeshima_day))
Verbal_conf
```
...
```{r}
Verbal_conf<- auto.arima(ritualized_day$`Verbal Conflict`, test="adf", xreg = cbind(dokdo_drill))

 cbind(broom::tidy(Verbal_conf)[5:11,], order=7:1) %>%
  ggplot(aes(x=estimate, y=fct_reorder(term, order))) + geom_point(color="darkseagreen4") + 
  geom_vline(xintercept = 0, color="grey30", size = 0.1) + 
  #geom_vline(xintercept = -2, color="grey80", size = 0.1) + 
  #geom_vline(xintercept = 2, color="grey80", size = 0.1) + 
...
material_conf <- arima(ritualized_day$`Material Conflict`, c(1,0,2), 
                       seasonal = list(order = c(0, 0, 1), period = 7),
                xreg = takeshima_day)

 cbind(broom::tidy(material_conf)[6:17,], order=12:1) %>%
  ggplot(aes(x=estimate, y=fct_reorder(term, order))) + geom_point(color="darkseagreen4") + 
...
somatrix <- cbind(
model.matrix( ~ `South Korean President` - 1, data=takeshima_p[,c(13)]),
model.matrix( ~ `Japanese Prime Minister` - 1, data=takeshima_p[,c(14)])) 
...
takeshima_day_step <- ritualized_day %>%
  mutate(
  `Passage of T Day Ordinance (2005)` = 
    ifelse(as_date(time) >= as_date("2005/02/21"),1,0),
  `1st Takeshima Day (2006)` = 
    ifelse(as_date(time) >= as_date("2006/02/21"),1,0),
  `2nd Takeshima Day (2007)` = 
    ifelse(as_date(time) >= as_date("2007/02/21"),1,0),
  `3nd Takeshima Day (2008)` = 
    ifelse(as_date(time) >= as_date("2008/02/21"),1,0),
  `4th Takeshima Day (2009)` = 
    ifelse(as_date(time) >= as_date("2009/02/21"),1,0),
  `5th Takeshima Day (2010)` = 
    ifelse(as_date(time) >= as_date("2010/02/21"),1,0),
  `6th Takeshima Day (2011)` = 
    ifelse(as_date(time) >= as_date("2011/02/21"),1,0),
  `7th Takeshima Day (2012)` = 
    ifelse(as_date(time) >= as_date("2012/02/21"),1,0),
  `Lee Myung-bak's Dokdo Visit` = 
  ifelse(as_date(time) >= as_date("2012/08/09"),1,0),
  `8th Takeshima Day (2013)` = 
    ifelse(as_date(time) >= as_date("2013/02/21"),1,0),
  `9th Takeshima Day (2014)` = 
    ifelse(as_date(time) >= as_date("2014/02/21"),1,0),
  `10th Takeshima Day (2015)` = 
    ifelse(as_date(time) >= as_date("2015/02/21"),1,0)) %>% 
  dplyr::select(-c(1:7))
...
Verbal_conf<- auto.arima(ritualized_day$`Verbal Conflict`, test="adf", xreg = cbind(takeshima_day_step))
Verbal_conf
```
...
```{r, echo=TRUE}
bobo <- sim_sum %>% filter(date >= as_date("2001-01-02"), date <= as_date("2015-09-30"))
Verbal_conf<- auto.arima(ritualized_day$`Verbal Conflict`, test="adf", 
                         xreg = bobo$HighR)
summary(Verbal_conf)
Verbal_conf<- auto.arima(ritualized_day$`Verbal Conflict`, test="adf", 
                         xreg = bobo$LowR)
...
```{r, warning=FALSE, message=FALSE}
library(vars)
VARselect(ritualized_day$`Verbal Conflict`, lag.max = 10, type="both")
firstVar <- VAR(cbind(a = ritualized_day$`Verbal Conflict`, b = bobo$HighR, c=bobo$LowR), p=7)
irf(firstVar, impulse="b", response="a") %>% plot()
irf(firstVar, impulse="c", response="a") %>% plot()
```
